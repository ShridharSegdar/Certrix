...
def sign_csr_with_ca_and_defaults(csr: x509.CertificateSigningRequest,
                                  ca_key,
                                  ca_cert,
                                  years: int) -> x509.Certificate:
    """
    Take a CSR and sign it with the persistent CA, applying default
    KeyUsage / EKU / SKI / AKI if not present.
    """
    now = datetime.datetime.utcnow()
...
def csr_preview():
    """
    Preview CSR subject (CN, O, OU, etc.) and key info.
    Accepts:
      - csr_file (uploaded file) OR
      - csr_text (pasted PEM)
...
def renew_bulk():
    """
    Bulk renewal:
      - Accepts a ZIP file upload containing multiple .csr/.pem/.der files.
      - Signs each CSR with the persistent CA.
      - Returns a ZIP containing <CN>.cer for every valid CSR.
    """
    zip_file = request.files.get("csr_zip")
    if not zip_file or not zip_file.filename:
        return abort(400, "csr_zip (ZIP file) is required")

    # validity (years) for all renewed certs
    try:
        years = int(request.form.get("years", "1") or 1)
        years = max(1, min(years, 50))
    except Exception:
        ...
        with zipfile.ZipFile(in_mem, "r") as zin:
            ...
            with zipfile.ZipFile(out_mem, "w", compression=zipfile.ZIP_DEFLATED) as zout:
                for info in zin.infolist():
                    ...
                    name = info.filename
                    lower = name.lower()
                    if not (lower.endswith(".csr") or lower.endswith(".pem") or lower.endswith(".der")):
                        continue
                    ...
                    try:
                        ...
                        # skip invalid CSR but keep processing others
                        continue

                    # derive CN-based name
                    try:
                        cn_attr = csr.subject.get_attributes_for_oid(NameOID.COMMON_NAME)
                        cn_from_csr = cn_attr[0].value if cn_attr else ""
                    except Exception:
                    ...
                    fallback = Path(name).stem  # original filename stem
